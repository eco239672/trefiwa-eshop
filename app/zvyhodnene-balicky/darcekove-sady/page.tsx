...
export default function ZvyhodneneBalickyPage() {
  ...
  useEffect(() => {
    getProductsBySubCategory("Darčekové sady")
      .then((data: any) => {
        setProducts(data);
        setIsLoading(false);
      })
      .catch((err: any) => {
        console.error("Chyba pri načítaní darčekových sád:", err);
        setIsLoading(false);
      });
  }, []);
    ...
    <main className="min-h-screen bg-[#FAF4E8] text-[#3D4035] flex flex-col">
      ...
      <div className="flex-grow max-w-7xl mx-auto w-full px-6 py-16">
        ...
        {isLoading ? (
          ...
          /* TEXT PRI PRÁZDNEJ KATEGÓRII */
          <p className="text-center text-[#A3A697] py-10">
            V kategórii Darčekové sady zatiaľ nie sú žiadne produkty.
          </p>
        ) : (
          <>
            {/* Zoraďovací panel */}
      ...
      {/* SEO 5 PODSEKCIÍ DARČEKOVÝCH SÁD */}
        ...
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-xs font-bold uppercase tracking-widest text-[#8A9A5B] block mb-2">
              Pre radosť vašich blízkych
            </span>
            <h3 className="text-3xl md:text-4xl font-bold text-[#2C2E26]">
              Zistite viac o našich darčekových sadách
            </h3>
          </div>
          <div className="space-y-12 text-[#6B6E56] leading-relaxed text-base md:text-lg">
            ...
            >
              <h4 className="text-xl font-semibold text-[#3D4035] mb-3 flex items-center gap-3">
                ...
                Pre koho sú naše sady vhodné?
              </h4>
              <p>Miesto pre váš text k prvej podsekcii darčekových sád...</p>
            </div>
            <div
              id="sekcia-2"
              className="p-6 bg-[#F9F8F6] rounded-xl border border-[#E8E6DF] scroll-mt-32"
