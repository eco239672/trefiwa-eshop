...
export default function CajnikyPage() {
  const [products, setProducts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [sortOption, setSortOption] = useState("najpredavanejsie");

  useEffect(() => {
    // Ak sa kategória v databáze volá inak (napr. "Cajniky" bez dĺžňov), prepíš to tu
    getProductsBySubCategory("Čajníky")
      ...
      .catch((err: any) => {
        console.error("Chyba pri načítaní čajníkov:", err);
        setIsLoading(false);
      });
  }, []);
      ...
      <div className="flex-grow max-w-7xl mx-auto w-full px-6 py-16">
        ...
        <section className="text-center max-w-4xl mx-auto mb-12">
          ...
          <p className="text-lg md:text-xl text-[#6B6E56] leading-relaxed">
            Čajník je tichým srdcom kuchyne, ktorý premieňa vodu na teplé
            hrejivé objatie a dúšok relaxu. Vyberte si ten pravý pre váš domov.
          </p>
        </section>
        ...
        {isLoading ? (
          ...
          <p className="text-center text-[#A3A697] py-10">
            V kategórii Čajníky zatiaľ nie sú žiadne produkty.
          </p>
        ) : (
          <>
            {/* Zoraďovací panel */}
          ...
          <div className="space-y-12 text-[#6B6E56] leading-relaxed text-base md:text-lg">
            ...
            >
              <h4 className="text-xl font-semibold text-[#3D4035] mb-3 flex items-center gap-3">
                ...
                Ako si vybrať správny čajník?
              </h4>
              <p>Miesto pre váš text k prvej podsekcii o čajníkoch...</p>
            </div>
            <div
              id="sekcia-2"
              className="p-6 bg-[#F9F8F6] rounded-xl border border-[#E8E6DF] scroll-mt-32"
            ...
            >
              <h4 className="text-xl font-semibold text-[#3D4035] mb-3 flex items-center gap-3">
                ...
                Aké typy čajov v nich pripravovať
              </h4>
