...
export default function CierneCajePage() {
  const [cart, setCart] = useState<Product[]>([]);
  const [products, setProducts] = useState<Product[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    getProductsBySubCategory("Čierne čaje")
      .then((data) => {
        setProducts(data);
        setIsLoading(false);
      })
      ...
      <div className="flex-grow max-w-7xl mx-auto w-full px-6 py-16">
        <section className="text-center max-w-4xl mx-auto mb-16">
          ...
          <p className="text-lg md:text-xl text-[#6B6E56] leading-relaxed">
            Silné, povzbudivé a plné chuti. Ideálna voľba pre perfektné
            naštartovanie vášho dňa.
          </p>
        </section>
        ...
        {isLoading ? (
          ...
          <p className="text-center text-[#A3A697] py-10">
            V kategórii Čierne čaje zatiaľ nie sú žiadne produkty.
          </p>
        ) : (
                ...
                <div className="w-full h-48 bg-[#EFEFEA] rounded-md mb-4 flex items-center justify-center text-[#A3A697] overflow-hidden group-hover:opacity-90 transition-opacity">
                  {(product as any).imageUrl ? (
                    <div
                      ...
                      style={{
                        backgroundImage: `url(${(product as any).imageUrl})`,
                      }}
                    ></div>
                  ) : (
      ...
      <footer className="bg-[#2C2E26] text-[#D5D3C9] pt-16 pb-8 mt-auto">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <div>
            <Link
              href="/"
              className="text-3xl font-bold tracking-widest text-[#8A9A5B] hover:text-[#A3A697] transition-colors block mb-6"
            >
              TREFIWA
            </Link>
            <p className="text-sm leading-relaxed text-[#A3A697]">
              Vaša denná dávka prírody. Ponúkame výber tých najkvalitnejších
              sypaných čajov a zdravých potravín pre váš vyvážený životný štýl.
            </p>
          </div>
          <div>
