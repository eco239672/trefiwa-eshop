...
export default function JednorazovyEkoRiadPage() {
  ...
  useEffect(() => {
    // ⚠️ Skontroluj si presný názov podkategórie vo svojej databáze
    getProductsBySubCategory("Jednorazový eko-riad")
      .then((data: any) => {
        setProducts(data);
        setIsLoading(false);
      })
      .catch((err: any) => {
        console.error("Chyba pri načítaní eko riadu:", err);
        setIsLoading(false);
      });
  }, []);
    ...
    <main className="min-h-screen bg-[#FAF4E8] text-[#3D4035] flex flex-col">
      <div className="flex-grow max-w-7xl mx-auto w-full px-6 py-16">
        ...
        {isLoading ? (
          ...
          <p className="text-center text-[#A3A697] py-10">
            V kategórii Jednorazový eko riad zatiaľ nie sú žiadne produkty.
          </p>
        ) : (
          <>
            {/* Zoraďovací panel */}
          ...
          <div className="space-y-6 text-[#6B6E56] leading-relaxed text-base md:text-lg">
            ...
            >
              <h4 className="text-xl font-semibold text-[#3D4035] mb-3 flex items-center gap-3">
                ...
                Dizajn na nerozoznanie od klasiky
              </h4>
