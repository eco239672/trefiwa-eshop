...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await submitCheckout({
      ...
      pickupPoint: delivery.requiresPickupPoint && pickupPoint ? { id: pickupPoint.id } : undefined,
        ...
        {cart.length === 0 ? (
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="flex-1 space-y-6">
              ...
              <section className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-[#E8E6DF]"><h2 className="text-xl font-bold text-[#2C2E26] mb-4">Spôsob platby</h2><label className="flex items-center gap-3 p-4 rounded-xl border-2 border-[#5C6B46] bg-[#F2F1EC]"><input type="radio" name="paymentMethod" checked readOnly /><span><strong className="block">Bankový prevod</strong><span className="text-xs text-[#8A9A5B]">Platobné pokyny a referencia objednávky sa zobrazia po vytvorení objednávky.</span></span></label><div className="flex items-center gap-3 p-4 rounded-xl border border-[#E8E6DF] opacity-60 mt-3" aria-disabled="true"><input type="radio" name="paymentMethod" disabled /><span><strong className="block">Platba kartou online</strong><span className="text-xs text-[#8A9A5B]">Dočasne nedostupné – platobná brána ešte nie je pripojená.</span></span></div></section>
            </div>
